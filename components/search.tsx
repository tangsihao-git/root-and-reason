...
export function Search(){
 ...
 const { default: entries } = await import('@/lib/generated/search.json');
 ...
 <div className="search-results" aria-live="polite">{!query.trim()?<p>输入关键词搜索文档。</p>:busy?<p>正在搜索…</p>:results.length?results.map(r=><Link prefetch={false} key={r.url} href={base&&r.url.startsWith(base+'/')?r.url.slice(base.length):r.url} onClick={()=>{dialog.current?.close();setExpanded(false)}}><strong>{r.title}</strong><span>{r.excerpt}</span></Link>):<p>没有找到相关内容。</p>}</div>
 </dialog>
 ...
 {expanded&&!!query.trim()&&<div id="library-search-results" className="inline-search-panel"><div className="search-results" aria-live="polite">{busy?<p>正在搜索…</p>:results.length?results.map(r=><Link prefetch={false} key={r.url} href={base&&r.url.startsWith(base+'/')?r.url.slice(base.length):r.url} onClick={()=>setExpanded(false)}><strong>{r.title}</strong><span>{r.excerpt}</span></Link>):<p>没有找到相关内容，试试更简短的关键词。</p>}</div></div>}</div>;
}
